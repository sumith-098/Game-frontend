...
const Home = ({ playerName, setPlayerName, gameMode, setGameMode }) => {
  ...
  const GAME_API_URL = 'https://game-c2j9.onrender.com/api/game';

  const CHAT_API_URL="https://game-chat-service.onrender.com"
  ...
  const handleCreateOrJoin = async (action) => {
    ...
    try {
      ...
      const endpoint = `${GAME_API_URL}/${action === 'CREATE' ? 'create' : 'join'}`;
